{ pkgs, utils, stableDiffusionCpp ? pkgs.stable-diffusion-cpp-rocm }:
...
let
  ...
  generateCmd = pkgs.writeShellApplication {
    name = "generate";
    runtimeInputs = [ stableDiffusionCpp pkgs.jq pkgs.coreutils utils.ensureModels utils.jsonToArgs ];
    text = ''
